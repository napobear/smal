...
/* Simple macro assembler and linkage editor with a 32 bit word.
   ...
   rev 5/21/24 support A=:1 as non-redefinable definition, forbid .=:1
...
/* version control */
#define SMALversion "SMAL32 (rev  5/24)              "
...
static void labeldef() {
	...
	symptr symbol = lookup(lex.pos, lex.lim);
	...
	} /* else the table was full, no need for error message */
...
static void definition() {
	...
	/* assume that (lex.typ = id) and (next.typ = eq) */
	int idpos = lex.pos;
	int idlim = lex.lim;
	symptr symbol = lookup(idpos, idlim);
	association *WITH;

	nextlex();   /* read over id */
	nextlex();   /* read over eq */
	if (lex.typ != colon) { /* redefinable */
		if (((1L << lab) & symtab[symbol - 1].use) != 0)
			errmsg(muldef, idpos, idlim);
		expresbal();
		if (symbol <= 0) /* table was full, error already reported */
			return;
		WITH = &symtab[symbol - 1];
		WITH->use |= 1L << setyet;
		if (exprdef) {
	...
	} else { /* not redefinable, akin to a label definition */
		nextlex();   /* read over colon */
		expresbal();
		if (symbol <= 0) /* table was full, error already reported */
			return;
		...
		if (((1L << setyet) & WITH->use) != 0) {
			...
			if (WITH->val.offset != expr.offset ||
			    WITH->val.base != expr.base) {
				errmsg(phase, idpos, idlim);
			}
		} else {
			WITH->val = expr;
		}
...
static void origin() {
	...
	start = lex.pos;
	nextlex();   /* skip eq */
	if (lex.typ == colon) { /* =: not allowed here */
		errmsg(baddir, start, lex.lim);
		nextlex();   /* skip colon */
	}
	expresbal();
	loc = expr;
