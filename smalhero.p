program {$p-} smalhero(lst,obj,inp1,inp2,inp3,input,output);
	...
	rev started 10/15/82 for support of icap instruction set.
	rev started 3/25/83 for transport to UNIX.
	rev started 4/1/83 for support of HERO-1 robot (M6800 based).
     }
	...
	works in berkeley pascal on vax unix (integers and chars as hull
	  v-mode), after converting all non-quoted text to lower case.
     }
	...
	with the same root name but a .lst suffix, and the object on
	a file with a .obj suffix.      				}
...
const symsize = 300 { number of symbols allowed in symboltable };
      opcodes = 200 { opcodes and macro names allowed in optab
			   includes 19 for basic smal opcode set,
				    72 for 6800 opcodes,
				    35 for HERO-1 opcodes,
				    74 for user defined macros };
      poolsize = 4000{ chars in stringpool; should be greater than the
		       ...
		       popint   				       };
      regsym = 2    { offset of null register symbol in pool (6800) };
      ...
      listcol = 31  { last column of listing used for object code info, should
      ...
      listcodes = 5 { object code items (bytes,words) listed on a line, set so
		       that: ((4 * listcodes) + 14) > listcol };
      filelen = 32  { maximum length of symbolic file name };
...
type filename = packed array [1..filelen] of char;
     ...
     optypes = (operr  { error }
	       ...
	       ,oppage { page eject directive }
               ,opstart{ directive to specify starting address }
	       ,opend  { end of file }
 
	       { macro call type }
	       ,opmcall{ macro call }
 
	       { basic 6800 machine instruction types }
	       ,opmtoreg { memory to register }
	       ,opregtom { register to memory }
	       ,opmtoimp { memory to implicit }
	       ,opimptom { implicit to memory }
	       ,opunary  { unary operators }
	       ,opimp    { all operands implicit }
	       ,opstk    { stack operations }
	       ,oprel    { relative addressing }
	       ,opjmp    { jump instructions }

	       { special HERO instruction formats }
	       ,optime   { sleep and pause instructions }
	       ,opijmp   { immediate jump instructions }
	       ,opmmove  { motor move instructions }
	       ,opmccw   { motor move command word directive }
	       ,opmall   { move all instructions }
	       );
...
var
    ...
    indsym: poolref     		 { 6800 index register name };
    ...
    { information used for pagination of listing file }
    linesper,lineonpg: integer    { lines per page and line on page };
    titlebuf,sbttlbuf: linetype   { title and subtitle buffers      };
    titlelen,sbttllen: inbufptr   { lengths of title and subtitle   };
...
{ inside smal, procedures for doing output format conversion }
    ...
    procedure writetohex( var f:text; v,l:integer);
...
{ inside smal, procedures for symbol table management }
    ...
    begin { opinit };
	 ...
	 op('PAGE    ',oppage   ,0);
         op('S       ',opstart  ,0);
	 op('END     ',opend    ,0);
	 { 6800 machine instruction mnemonics }
	 op('ADD     ',opmtoreg ,tohex('008B'));
	 op('ADC     ',opmtoreg ,tohex('0089'));
	 op('AND     ',opmtoreg ,tohex('0084'));
	 op('BIT     ',opmtoreg ,tohex('0085'));
	 op('CMP     ',opmtoreg ,tohex('0081'));
	 op('EOR     ',opmtoreg ,tohex('0088'));
	 op('LDA     ',opmtoreg ,tohex('0086'));
	 op('ORA     ',opmtoreg ,tohex('008A'));
	 op('SUB     ',opmtoreg ,tohex('0080'));
	 op('SBC     ',opmtoreg ,tohex('0082'));
	 op('STA     ',opregtom ,tohex('0087'));
	 op('CPX     ',opmtoimp ,tohex('008C'));
	 op('LDX     ',opmtoimp ,tohex('00CE'));
	 op('LDS     ',opmtoimp ,tohex('008E'));
	 op('STX     ',opimptom ,tohex('00CF'));
	 op('STS     ',opimptom ,tohex('008F'));
	 op('CLR     ',opunary  ,tohex('004F'));
	 op('COM     ',opunary  ,tohex('0043'));
	 op('NEG     ',opunary  ,tohex('0040'));
	 op('DEC     ',opunary  ,tohex('004A'));
	 op('INC     ',opunary  ,tohex('004C'));
	 op('ROL     ',opunary  ,tohex('0049'));
	 op('ROR     ',opunary  ,tohex('0046'));
	 op('ASL     ',opunary  ,tohex('0048'));
	 op('ASR     ',opunary  ,tohex('0047'));
	 op('LSR     ',opunary  ,tohex('0044'));
	 op('TST     ',opunary  ,tohex('004D'));
	 op('ABA     ',opimp    ,tohex('001B'));
	 op('CBA     ',opimp    ,tohex('0011'));
	 op('DAA     ',opimp    ,tohex('0019'));
	 op('SBA     ',opimp    ,tohex('0010'));
	 op('TAB     ',opimp    ,tohex('0016'));
	 op('TBA     ',opimp    ,tohex('0017'));
	 op('DEX     ',opimp    ,tohex('0009'));
	 op('DES     ',opimp    ,tohex('0034'));
	 op('INX     ',opimp    ,tohex('0008'));
	 op('INS     ',opimp    ,tohex('0031'));
	 op('TXS     ',opimp    ,tohex('0035'));
	 op('TSX     ',opimp    ,tohex('0030'));
	 op('NOP     ',opimp    ,tohex('0001'));
	 op('RTI     ',opimp    ,tohex('003B'));
	 op('RTS     ',opimp    ,tohex('0039'));
	 op('SWI     ',opimp    ,tohex('003F'));
	 op('WAI     ',opimp    ,tohex('003E'));
	 op('CLC     ',opimp    ,tohex('000C'));
	 op('CLI     ',opimp    ,tohex('000E'));
	 op('CLV     ',opimp    ,tohex('000A'));
	 op('SEC     ',opimp    ,tohex('000D'));
	 op('SEI     ',opimp    ,tohex('000F'));
	 op('SEV     ',opimp    ,tohex('000B'));
	 op('TAP     ',opimp    ,tohex('0006'));
	 op('TPA     ',opimp    ,tohex('0007'));
	 op('PSH     ',opstk    ,tohex('0036'));
	 op('PUL     ',opstk    ,tohex('0032'));
	 op('BRA     ',oprel    ,tohex('0020'));
	 op('BCC     ',oprel    ,tohex('0024'));
	 op('BCS     ',oprel    ,tohex('0025'));
	 op('BEQ     ',oprel    ,tohex('0027'));
	 op('BGE     ',oprel    ,tohex('002C'));
	 op('BGT     ',oprel    ,tohex('002E'));
	 op('BHI     ',oprel    ,tohex('0022'));
	 op('BLE     ',oprel    ,tohex('002F'));
	 op('BLS     ',oprel    ,tohex('0023'));
	 op('BLT     ',oprel    ,tohex('002D'));
	 op('BMI     ',oprel    ,tohex('002B'));
	 op('BNE     ',oprel    ,tohex('0026'));
	 op('BVC     ',oprel    ,tohex('0028'));
	 op('BVS     ',oprel    ,tohex('0029'));
	 op('BPL     ',oprel    ,tohex('002A'));
	 op('BSR     ',oprel    ,tohex('008D'));
	 op('JMP     ',opjmp    ,tohex('004E'));
	 op('JSR     ',opjmp    ,tohex('008D'));
	 { Heath HERO-1 robot control language opcodes }
	 op('ADM     ',opimp    ,tohex('0002'));
	 op('ASM     ',opimp    ,tohex('0003'));
	 op('AAM     ',opimp    ,tohex('0004'));
	 op('ASP     ',opimp    ,tohex('0005'));
	 op('BBB     ',oprel    ,tohex('001C'));
	 op('BSB     ',oprel    ,tohex('001D'));
	 op('BAB     ',oprel    ,tohex('001E'));
	 op('BSP     ',oprel    ,tohex('001F'));
	 op('ZERO    ',opimp    ,tohex('0021'));
	 op('RTE     ',opimp    ,tohex('003A'));
	 op('CRL     ',opimp    ,tohex('003F')) {same as SWI};
	 op('ELD     ',opimp    ,tohex('0041'));
	 op('ESD     ',opimp    ,tohex('0042'));
	 op('EUR     ',opimp    ,tohex('0045'));
	 op('EMD     ',opimp    ,tohex('004B'));
	 op('ED      ',opimp    ,tohex('004E'));
	 op('DLD     ',opimp    ,tohex('0051'));
	 op('DSD     ',opimp    ,tohex('0052'));
	 op('DMD     ',opimp    ,tohex('0055'));
	 op('DUR     ',opimp    ,tohex('005B'));
	 op('DD      ',opimp    ,tohex('005E'));
	 op('SPC     ',opjmp    ,tohex('0041'));
	 op('SPW     ',opjmp    ,tohex('0042'));
	 op('CML     ',opimp    ,tohex('0083'));
	 op('SLEEP   ',optime   ,tohex('0087'));
	 op('PAUSE   ',optime   ,tohex('008F'));
	 op('JSP     ',opijmp   ,tohex('00BF'));
	 op('MMWAI   ',opmmove  ,tohex('00C3'));
	 op('MMCAI   ',opmmove  ,tohex('00CC'));
	 op('MMWRI   ',opmmove  ,tohex('00D3'));
	 op('MMCRI   ',opmmove  ,tohex('00DC'));
	 op('MMWA    ',opjmp    ,tohex('00C3'));
	 op('MMCA    ',opjmp    ,tohex('00CC'));
	 op('MCCW    ',opmccw   ,0            );
	 op('MMALL   ',opmall   ,tohex('00FD'));
...
{ inside smal, main assembly procedure for one pass through source }
    ...
    const immmode = 0   { 6800 immediate addressing mode };
	  dirmode = 1   { 6800 direct (short) addressing mode };
	  indmode = 2   { 6800 indexed addressing mode };
	  extmode = 3   { 6800 extended (long) addressing mode };
    ...
    type ermsg = (minermsg
		 ...
		 ,notreg { expression should be valid 6800 register }
		 ,notvar { 6800 addressing mode should not be immediate }
		 ,notind { 6800 index register expected }
		 ,maxermsg);
	...
	exprfwd:boolean          { set if expression contains forward refs };
	exprpos,exprlim:inbufptr { position of expression on line };
 
	{ info about 6800 operand addresses }
	reg: 0..1;
	addr: record
		   mode: 0..3;
		   base: poolref;
	...
	procedure settitle(var t:linetype { buffer holding title };
			   var l:inbufptr { length of stored title };
			       pos,lim:inbufptr { position in line }  );
	{ copy title from source line }
	var i:inbufptr;
	begin
	     for i := pos to lim do t[(i-pos)+1] := line[i];
	     l := (lim-pos)+1;
	end { settitle };

	procedure newpage;
	{  output formfeed to listing file  }
	var ff:char;
	begin
	...
	var i:inbufptr;
	    ...
	    var i:1..linelen;
	    begin
		 write(lst,'SMAL HERO ASSEMBLER, REV 9/4/84.   ');
	...
	begin
	     ...
	     if codelen > 0 then begin { list generated code }
		  ...
		  writetohex( lst, codeloc.offset,4 );
		  write( lst, ':' );
		  col := 14;
		  i := 0;
		  while i < codelen do begin { list each code item }
		       ...
		       with codebuf[i] do begin
			    ...
			    if nextcol > listcol then begin
				 ...
				 writetohex( lst, val.offset,2*siz );
				 col := nextcol;
				 if col < listcol then begin
				      write( lst, ' ' );
				      col := col + 1;
				 end;
			    end;
	     ...
	     if erset <> [] then begin
		  ...
		  { write markup under listed line in listing file }
		  for msg := minermsg to maxermsg do if msg in erset then begin
			 ...
			 muldef: write(lst,'MULTIPLE LABEL DEFINITION');
                         mulstt: write(lst,'MULTIPLE START DIRECTIVES');
			 ...
			 notreg: write(lst,'REGISTER EXPECTED        '){6800};
			 notvar: write(lst,'CONSTANT NOT ALLOWED     '){6800};
			 notind: write(lst,'INDEX REGISTER EXPECTED  '){6800};
		       end { case };
	...
	begin
	     if allowlist and (siz > 0) then begin
	...
	var acc: value  	  { the accumulator };
		...
		begin
		     ...
		     if lex.typ = num then begin  { got a number }
			  ...
			  if symbol > 0 then with symtab[symbol] do begin
			       ...
			       if not (setyet in use) then exprfwd := true;
			  end else begin
	    ...
	    begin { term }
		 if lex.typ in [plus,minus,notsym,qmark] then begin
		      { qmark was added above for 6800 register names }
		      op := lex;
		      ...
		      case op.typ of
		       ...
		       qmark: begin { handle 6800 register names }
	...
	begin
	     if (val > max) or (val < min) then begin
		  error( bounds, exprpos, exprlim );
		  val := min;
	     end;
	end { boundval };
 
	procedure absexpr( min,max:integer );
	begin
	     expresbal;
	     if expr.base <> abssym then begin
		  error( badrel, exprpos, exprlim );
		  expr.offset := min;
	     end else boundval( expr.offset, min, max );
	     expr.base := abssym;
	end { absexpr };
    ...
    { inside smal.onepass, processing of special 6800 operands }
 
	procedure forcereg;
	{ force expr to be legal 6800 register }
	begin
	     if expr.base <> regsym then begin
		  error( notreg, exprpos, exprlim );
		  expr.offset := 0;
	     end else boundval( expr.offset, 0, 1 );
	end { forcereg };
 
	procedure genaddr;
	{ evaluate general 6800 address, setting addr appropriately }
	begin
	     if lex.typ = pndsgn then begin { immediate mode }
		  nextlex;
		  addr.mode := immmode;
		  addr.bytes := 2;
		  expresbal;
		  ...
		  expresbal;
		  if (lex.typ = comma) or (lex.typ = id) then begin { indexed }
		       if lex.typ = comma then nextlex;
		       if not poolcmp( indsym, lex.pos, lex.lim )
			 then error(notind,lex.pos,lex.lim);
		       nextlex;
		       boundval( expr.offset, -128, 127 );
		       addr.mode := indmode;
		       addr.bytes := 1;
		  end else begin
		       addr.mode := extmode;
	     ...
	     addr.offset := expr.offset;
	     addr.base := expr.base;
	end { genaddr };
 
	procedure forcedst;
	{ force 6800 destination address, resetting addr as needed }
	begin
	     if addr.mode = immmode then begin
		  addr.mode := extmode;
		  error( notvar, exprpos, exprlim );
	     end;
	end { forcedst };

	procedure shortimm;
	{ force 6800 immediate constants to be short }
	begin
	     if addr.mode = immmode then begin
		  boundval( addr.offset, -128, 255 );
		  addr.bytes := 1;
	     end;
	end { shortimm };

	procedure allowdir;
	{ allow 6800 short direct addresses, resetting addr as needed }
	begin
	     if addr.mode = extmode then begin
		  if (addr.base=abssym) and exprdef and not exprfwd then begin
		       if (addr.offset >= 0) and (addr.offset <=255) then begin
			    addr.mode := dirmode;
			    addr.bytes := 1;
		       end;
		  end;
	...
	procedure allowregs;
	{ allow 6800 register references in address field }
	begin
	     if addr.mode = extmode then begin
		  if addr.base = regsym then begin
		       boundval( addr.offset, 0, 1 );
		       addr.mode := addr.offset;
		       addr.bytes := 0;
		  end;
	     end;
	end { allowregs };
    ...
    { inside smal.onepass, processing of special HERO-1 operands }

	procedure mccword;
	{ parse motor control command word for HERO-1, put value in expr }
	var motor, speed, direction, distance: integer;
	begin
	     absexpr( 0, 7 );
	     motor := expr.offset;
	     getcomma;
	     ...
	     getcomma;
	     absexpr( 0, 1 );
	     direction := expr.offset;
	     getcomma;
	     absexpr( 0, 1023 );
	     distance := expr.offset;
	     expr.offset := distance + 1024*direction + 2048*speed;
	     if motor > 3
	       then expr.offset := ((expr.offset + 8192*(motor-4)) - 32767) - 1
	       else expr.offset := expr.offset + 8192*motor;
	end { mccword };
    ...
    begin { onepass }
	 ...
	 letters := ['A'..'I']+['J'..'R']+['S'..'Z'] { broken for EBCDIC }
                   +['a'..'i']+['j'..'r']+['s'..'z'];
	 digits := ['0'..'9'];
	 ...
	 if allowlist then newpage { start listing on new page };
	 ...
	 while getlevel >= 1 do begin
	     ...
	     opelse, opelseif: findend;
		      opendif:;
			opint: begin { internal symbol definition }
				    internl;
				    while lex.typ = comma do begin
					 nextlex;
					 internl;
			...
			opext: begin { external symbol definition }
				    externl;
				    while lex.typ = comma do begin
					 nextlex;
					 externl;
			...
			opttl: begin
				    ...
				    lex.typ := eol {kill parse of line};
			       end;
		      opsbttl: begin { set listing subtitle }
				    if firstpass then begin
					 listline;
				    end else if allowlist then begin
			...
			opend: popget;
		     ...
		     opmtoreg: begin { 6800 memory to register instructions };
				    ...
				    putobj( 1, opval
					     + 64*reg
					     + 16*addr.mode, abssym );
				    putobj( addr.bytes,addr.offset,addr.base );
			       end;
		     opregtom: begin { 6800 register to memory instructions };
				    expresbal;
				    forcereg;
				    reg := expr.offset;
				    if lex.typ = comma then nextlex;
		     ...
		     opmtoimp: begin { 6800 memory to implicit instructions }
				    genaddr;
				    allowdir;
				    putobj( 1, opval
					     + 16*addr.mode, abssym );
		     ...
		     opimptom: begin { 6800 implicit to memory instructions }
				    genaddr;
				    allowdir;
				    forcedst;
				    putobj( 1, opval
		      ...
		      opunary: begin { 6800 unary operators }
				    genaddr;
				    forcedst;
				    allowregs;
				    putobj( 1, opval
			...
			opimp: begin { 6800 implicit operators }
				    putobj( 1, opval, abssym );
			       end;
			opstk: begin { 6800 stack operators }
				    expresbal;
				    forcereg;
				    putobj( 1, opval
					     + expr.offset, abssym );
			       end;
			oprel: begin { 6800 relative addressing }
				    expresbal;
				    expr.offset := expr.offset-(loc.offset+2);
				    putobj( 1, opval, abssym );
				    if loc.base <> expr.base then begin
					 error(badrel,exprpos,exprlim);
					 expr.offset := 0;
				    end else if (expr.offset > 127)
					     or (expr.offset < -128) then begin
					 error(bounds,exprpos,exprlim);
					 expr.offset := 0;
				    end;
				    putobj( 1, expr.offset, abssym );
			       end;
			opjmp: begin { 6800 jump and call instructions }
				    genaddr;
				    forcedst;
				    putobj( 1, opval
					     + 16*addr.mode, abssym );
		       ...
		       opijmp,       { HERO-1 absolute jump commands }
		       optime: begin { HERO-1 sleep and pause commands }
				    expresbal;
				    putobj( 1, opval, abssym );
				    putobj( 2, expr.offset, expr.base );
			       end;
		      opmmove: begin { HERO-1 immediate motor move commands }
				    mccword;
				    putobj( 1, opval, abssym );
				    putobj( 2, expr.offset, abssym );
			       end;
		       opmccw: begin { HERO-1 motor move command word drctve }
				    mccword;
				    putobj( 2, expr.offset, abssym );
			       end;
		       opmall: begin { HERO-1 motor move all command }
				    putobj( 1, opval, abssym );
				    absexpr( 0, 152 ) { extend };
				    putobj( 1, expr.offset, abssym );
				    getcomma;
                                    absexpr( 0, 134 ) { shoulder };
                                    putobj( 1, expr.offset, abssym );
                                    getcomma;
                                    absexpr( 0, 147 ) { wrist rotate };
                                    putobj( 1, expr.offset, abssym );
                                    getcomma;
                                    absexpr( 0, 165 ) { wrist pivot };
                                    putobj( 1, expr.offset, abssym );
                                    getcomma;
                                    absexpr( 0, 117 ) { grip };
                                    putobj( 1, expr.offset, abssym );
	 ...
	 if allowlist then begin { wind up with code generation work }
	      if (loc.base <> relsym) or (loc.offset <> maxrel) then begin
		   ...
		   genval(2,maxrel,relsym);
              ...
              if seenstart then begin
                   write(obj,'S');
                   genval(2,startloc.offset,startloc.base);
              end;
	      if ercount > 0 then writeln('ASSEMBLY ERROR COUNT =',ercount:1);
	 end;
    end { onepass };
...
begin { smal }
     ...
     if symfull then writeln(lst,'** SYMBOL TABLE OVERFLOWED **');
     if poolfull then writeln('** STRING POOL OVERFLOWED **');
     if poolfull then writeln(lst,'** STRING POOL OVERFLOWED **');
     if opfull then writeln('** MACRO NAME TABLE FULL **');
     if opfull then writeln(lst,'** MACRO NAME TABLE FULL **');
end { smalicap }.
